...
class Kr < Formula
    ...
    homepage "https://github.com/ysicing/kubectl-resource"
    version "${version}"

    if OS.mac?
      if Hardware::CPU.arm?
        url "https://github.com/ysicing/kubectl-resource/releases/download/#{version}/kr_darwin_arm64"
        sha256 "${macosarm64sha}"
      else
        url "https://github.com/ysicing/kubectl-resource/releases/download/#{version}/kr_darwin_amd64"
        sha256 "${macosamd64sha}"
      end
    elsif OS.linux?
      if Hardware::CPU.intel?
        url "https://github.com/ysicing/kubectl-resource/releases/download/#{version}/kr_linux_amd64"
        sha256 "${linuxamd64sha}"
      end
      if Hardware::CPU.arm? && Hardware::CPU.is_64_bit?
        url "https://github.com/ysicing/kubectl-resource/releases/download/#{version}/kr_linux_arm64"
